...
window.title('programming homework')
...
label_frame2 = LabelFrame(frame, text='Answers', font=('modern', 25, 'bold'), fg=COLOR2, bg=COLOR1)
label_frame2.grid(column=0, row=1)

# v_s = Scrollbar(label_frame2)
# v_s.pack(side=RIGHT, fill=Y)

label = Text(label_frame2, height=25, width=100, bg=COLOR2, font='terminal', wrap='none', pady=20, padx=20)
label.pack(padx=20, pady=20)
scrollbar = ttk.Scrollbar(label_frame2, command=label.yview)
...
def built_in_function():
    ...
    label.insert(END, '''[
    1)
    # 1 print() prints the given object
    print('this is homework of built in functions.')

    2)
    # 2 property() returns the property of attribute
    # use a method as an attribute
    class Employee:
    ...
    3)
    # 3 range() returns a sequence of integers
    print(range(1, 4))

    4)
    # 4 repr() returns a printable representation of the object
    original_string = repr("Hello, world!")
    print(original_string)  # Output: "'Hello, world!'"

    recreated_object = eval(original_string)
    print(recreated_object)  # Output: Hello, world!

    5)
    # 5 python reversed() returns the reversed iterator of a sequence
    a = [2, 3, 4]
    print(list(reversed(a)))

    6)
    # 6 python round() rounds a number
    print(round(3.5))

    7)
    # 7 python set() constructs a set
    list1 = [2, 3, 4]
    print(set(list1))

    8)
    # 8 python setattr() sts the value of an attribute of an object
    class Person:
        name = 'ahmad'
        age = 36
        country = 'afghanistan'
    ...
    def __init__(self, name):
        self.name = name
        Geeks.list_of_instance.append(self)
    ...
    emp = Employee("Alice", 101, "Developer")
    print(emp.name)
    ...
    30)
    # python compile()
    code = 'print('first line', 2+2)'
    code_2line ='print('second line', 2+3)'


    code_obj = compile(code, 'draft', 'exec')
    code_obj_2line = compile(code_2line, 'draft1', 'exec')
    ...
    print(dict([('a',2),('b',2)], c=3, d=4))

    34)
    # python dir()
    list = [1,2,3]
    print(dir(list)) #list methods and attributes available in an object

    class Car:
    def corolla(self):
        print('this is corolla')

        def runner(self):
            print('this is runner car')
    ...
    # python filter()
    list1 = [1, 2, 3, 4, 5]
    a = list(filter(lambda n: n % 2 == 0, list1))
    print(a)
    ...
    greeting = 'hello, {} my father name is {}'.format(name, fname)
    print(greeting)
    ...
    # frozen_set.add(5) produce error because in frozenset we cant add
    ...
    print(getattr(person, 'fname', 'dont have father name '))
    ...
    car = Car()
    has_brand_attribute = hasattr(car, 'brand')
    has_color_attribute = hasattr(car, 'color')
    print(has_brand_attribute, has_color_attribute)
    ...
    list2 = ['apple', 'banana', 'cat']  # another usage of iter() without of for loop.
    ...
    first_bite = memory_view[0]
    sub_view = memory_view[7:12]

    print(memory_view)
    print(first_bite)
    print(sub_view)
    print(sub_view.tobytes())
    print(first_bite.to_bytes())
    ...
    62)
    # python object()
    'Even without explicitly mentioning object() in your class definition, your class inherits from it by default,
        giving your objects access to methods'
    ...
    class vehicle(object):
        ...
        def info(self):
            print(f'this is the car {self.name} and year of {self.year}')

    car1 = vehicle('corolla', 2024)
    car1.info()
    ...
    64)
    # python open()
    with open('text.txt', mode='w') as t:
        t.write('ahmad and mahmod')
...
def factorial():
    label.delete('1.0', 'end')
    label.insert(END,"""def factorial(n):
    if n == 1 or n == 0:
        return 1
    else:
...
def fibonnaci():
    label.delete('1.0', 'end')
    label.insert(END, '''def fibonacci(n):
    if n <= 1:
        return n
    else:
...
class Manager(Employee):
    def __init__(self, department):
        super().__init__(name='ahmad', salary=2000)
        self.department = department


employee = Employee('ahmad', 2000)
print(employee.name, employee.salary)

manager = Manager(department='IT')
...
label_frame = LabelFrame(frame, text='Select One of These:', font=('modern', 25, 'bold'), fg=COLOR2, bg=COLOR1)
label_frame.grid(column=0, row=0, padx=20, pady=20)

button_function = Button(label_frame, text='Built in functions', command=built_in_function, font=('modern', 20, 'bold'), fg='black', activebackground=COLOR1, bg=COLOR2)
button_function.grid(column=0, row=0, padx=30, pady=30)
button_function = Button(label_frame, text='factorial', command=factorial, font=('modern', 20, 'bold'), fg='black', activebackground=COLOR1, bg=COLOR2, padx=40)
button_function.grid(column=1, row=0, padx=30, pady=30)
button_function = Button(label_frame, text='fibonnaci', command=fibonnaci, font=('modern', 20, 'bold'), fg='black', activebackground=COLOR1, bg=COLOR2, padx=40)
button_function.grid(column=2, row=0, padx=30, pady=30)
button_function = Button(label_frame, text='OOP & APPS', command= OOP_Apps, font=('modern', 20, 'bold'), fg='black', activebackground=COLOR1, bg=COLOR2, padx=40)
button_function.grid(column=3, row=0, padx=30, pady=30)
